...
import (
	...
	omni "github.com/voxgig-sdk/t-rex-vpn-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TRexVpnError); is {
				out := map[string]any{
					"name":    "TRexVpnError",
...
func omniCtx(arg any, client *sdk.TRexVpnSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TRexVpnSDK); is {
			client = live

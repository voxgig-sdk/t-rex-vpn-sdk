...
class TRexVpnMakeError
{
    public static function call(?TRexVpnContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TRexVpnContext([], null);
        }
        $op = $ctx->op ?? new TRexVpnOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TRexVpnResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TRexVpnError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TRexVpnError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof TRexVpnError) {

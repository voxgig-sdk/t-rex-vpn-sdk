...
class FtCtrl extends TRexVpnControl
{
...
class FtClient
{
    ...
    private TRexVpnUtility $utility;

    public function __construct(TRexVpnUtility $utility, array $options)
    {
    ...
    public function get_utility(): TRexVpnUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TRexVpnContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TRexVpnContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TRexVpnConfig::make_feature($name);
    ...
    private static function buildUrl(TRexVpnSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TRexVpnError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TRexVpnResponse($fetched);
        }
    ...
    private function fail(TRexVpnContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TRexVpnContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

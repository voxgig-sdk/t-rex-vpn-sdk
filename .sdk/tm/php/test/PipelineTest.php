...
{
    private static function utility(): TRexVpnUtility
    {
        return new TRexVpnUtility();
    }
    ...
    private static function ctx(array $over = []): TRexVpnContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TRexVpnContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TRexVpnOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TRexVpnError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TRexVpnOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TRexVpnMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TRexVpnOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TRexVpnMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TRexVpnOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TRexVpnMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TRexVpnSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TRexVpnMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TRexVpnResponse([]),
            'result' => new TRexVpnResult([]),
        ]);
        [, $err] = TRexVpnMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TRexVpnSpec([]),
            'response' => null,
            'result' => new TRexVpnResult([]),
        ]);
        [, $err] = TRexVpnMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TRexVpnSpec(['step' => 's']),
            'response' => new TRexVpnResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TRexVpnResult([]),
        ]);
        [, $err] = TRexVpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TRexVpnSpec(['step' => 's']),
            'response' => new TRexVpnResponse(self::resp(200, ['v' => 1])),
            'result' => new TRexVpnResult([]),
        ]);
        [, $err] = TRexVpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TRexVpnSpec(['step' => 's']),
            'response' => new TRexVpnResponse(self::resp(200, ['v' => 2])),
            'result' => new TRexVpnResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TRexVpnMakeResponse::call($ctx);
    ...
    {
        $preset = new TRexVpnResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TRexVpnSpec([]),
            'response' => new TRexVpnResponse([]),
            'result' => new TRexVpnResult([]),
        ...
        [$got, $err] = TRexVpnMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TRexVpnResult([])]);
        [, $err] = TRexVpnMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TRexVpnSpec([]), 'result' => null]);
        [, $err] = TRexVpnMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TRexVpnSpec(['step' => 's']),
            'result' => new TRexVpnResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TRexVpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TRexVpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TRexVpnSpec(['step' => 's']),
            'result' => new TRexVpnResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TRexVpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TRexVpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TRexVpnResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TRexVpnSpec([]),
            'result' => new TRexVpnResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TRexVpnMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TRexVpnMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TRexVpnError('boom', 'boom');
        $utility->fetcher = function (TRexVpnContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TRexVpnSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TRexVpnMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TRexVpnContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TRexVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TRexVpnResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TRexVpnContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TRexVpnSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TRexVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TRexVpnResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TRexVpnSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TRexVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TRexVpnMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TRexVpnMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TRexVpnResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TRexVpnDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TRexVpnResult(['ok' => false])]);
        $this->expectException(TRexVpnError::class);
        TRexVpnDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TRexVpnResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TRexVpnMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TRexVpnMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TRexVpnBaseFeature();
        $b = new TRexVpnBaseFeature();
        TRexVpnFeatureAdd::call($ctx, $a);
        TRexVpnFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TRexVpnBaseFeature
    {
        $f = new TRexVpnBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TRexVpnFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TRexVpnContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TRexVpnSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TRexVpnPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TRexVpnResponse(['headers' => null]),
            'result' => new TRexVpnResult([]),
        ]);
        TRexVpnResultHeaders::call($ctx);

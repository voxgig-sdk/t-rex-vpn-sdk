...
[MCP](https://modelcontextprotocol.io) server exposing the TRexVpn SDK as
two agent tools — `t-rex-vpn_list` and `t-rex-vpn_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/t-rex-vpn-mcp)
make build

# 2. Provide credentials via the environment
export T_REX_VPN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user t-rex-vpn \
  -- /absolute/path/to/t-rex-vpn-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./t-rex-vpn-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// t-rex-vpn_list: first page of records
{ "entity": "authentication" }
{ "entity": "authentication", "query": { } }

// t-rex-vpn_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/t-rex-vpn-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user t-rex-vpn \
     -- "$PWD"/dist/*/t-rex-vpn-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `t-rex-vpn_list` and `t-rex-vpn_load` tools now appear
   in new sessions. Ask the agent to *"list authentication using t-rex-vpn"*
   and it calls `t-rex-vpn_list` with `{"entity":"authentication"}`.
...
```sh
export T_REX_VPN_APIKEY=sk_live_xxx            # API key
export T_REX_VPN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `t-rex-vpn_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `t-rex-vpn_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `T_REX_VPN_APIKEY` | API key sent with every request. |
| `T_REX_VPN_BASE` | Optional override of the API base URL. |
...
```sh
./t-rex-vpn-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"t-rex-vpn_load","arguments":{"entity":"authentication","query":{"id":1}}}}'
```

# TRexVpn SDK test runner

from __future__ import annotations
import os
import json

from trexvpn_sdk.utility.voxgig_struct import voxgig_struct as vs


class TRexVpnTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TRexVpnTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TRexVpnTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TRexVpnTestRunner.getenv("T_REX_VPN_TEST_LIVE")
        override = TRexVpnTestRunner.getenv("T_REX_VPN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TRexVpnTestRunner.getenv(key)
        ...
        explain = TRexVpnTestRunner.getenv("T_REX_VPN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TRexVpnTestRunner._test_control is not None:
            return TRexVpnTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TRexVpnTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TRexVpnTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TRexVpnTestRunner.load_test_control()
...
def load_env_local():
    TRexVpnTestRunner.load_env_local()


def env_override(m):
    return TRexVpnTestRunner.env_override(m)


def entity_data(v):
    return TRexVpnTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TRexVpnTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TRexVpnTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TRexVpnTestRunner.load_test_control()


def live_client_options():
    return TRexVpnTestRunner.live_client_options()


def live_delay_ms():
    return TRexVpnTestRunner.live_delay_ms()

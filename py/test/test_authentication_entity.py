    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TREXVPN_TEST_AUTHENTICATION_ENTID JSON to run live")
        client = setup["client"]
...
def _authentication_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "TREXVPN_TEST_AUTHENTICATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TREXVPN_TEST_AUTHENTICATION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("TREXVPN_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
                "apikey": env.get("TREXVPN_APIKEY"),
            },
            extra or {},
        ])
        client = TRexVpnSDK(helpers.to_map(merged_opts))

    _live = env.get("TREXVPN_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("TREXVPN_TEST_EXPLAIN") == "TRUE",
        "live": _live,

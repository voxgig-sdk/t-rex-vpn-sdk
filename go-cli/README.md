...
boru-driven command-line client **and** interactive REPL for the TRexVpn
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/t-rex-vpn-cli)
make build
...
T_REX_VPN_BASE=https://api.example.com ./t-rex-vpn-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/t-rex-vpn-cli
   ```
...
Run with no arguments to open a REPL (prompt `t-rex-vpn>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/t-rex-vpn-cli`. |
...
(`./t-rex-vpn-cli <expr>`) and interactively (the REPL), and expressions compose the

// Command TRexVpn-cli is a CLI + REPL for the TRexVpn SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      T_REX_VPN_APIKEY (api key), T_REX_VPN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: T_REX_VPN_APIKEY carries the API key and
	// T_REX_VPN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("T_REX_VPN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("T_REX_VPN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTRexVpnSDK(opts)

...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.TREXVPN_TEST_LIVE ||
    'TRUE' === process.env.TREXVPN_TEST_OVERRIDE
  ) {
  ...
  m.TREXVPN_TEST_EXPLAIN = process.env.TREXVPN_TEST_EXPLAIN || m.TREXVPN_TEST_EXPLAIN

...
function isRunnable(code) {
    return (/new\s+TRexVpnSDK\b/.test(code) ||
        /\bTRexVpnSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+TRexVpnSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/TRexVpnSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);

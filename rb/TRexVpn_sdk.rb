...
require_relative 'TRexVpn_types'
...
class TRexVpnSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TRexVpnHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TRexVpnHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TRexVpnFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TRexVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TRexVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TRexVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TRexVpnError.new(
        "#{op}_allow",
        "TRexVpnSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = TRexVpnHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TRexVpnError.new(
        "graphql_error", "TRexVpnSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

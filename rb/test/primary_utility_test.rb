...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = TRexVpnOmni.make_runner(TEST_JSON_FILE, TRexVpnSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : TRexVpnOmni.errify(err)
    end

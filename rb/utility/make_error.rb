...
module TRexVpnUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = TRexVpnContext.new({}, nil)
    end
    op = ctx.op || TRexVpnOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || TRexVpnResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(TRexVpnError) ? err.msg : err.to_s
    ...
    sdk_err = TRexVpnError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(TRexVpnError)

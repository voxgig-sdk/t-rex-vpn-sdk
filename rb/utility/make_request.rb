...
module TRexVpnUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = TRexVpnResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TRexVpnResponse.new(fetched)
    else

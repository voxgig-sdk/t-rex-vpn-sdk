...
class TRexVpnNetsimFeature extends TRexVpnBaseFeature
{
    ...
    private function _simulate(TRexVpnContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TRexVpnContext $ctx, array $applied): void
    {
        $client = $this->client;

...
TRexVpnSDK._make_feature = _make_feature


function TRexVpnSDK.new(options)
  local self = setmetatable({}, TRexVpnSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TRexVpnSDK:options_map()
...
function TRexVpnSDK:get_utility()
  return Utility.copy(self._utility)
end


function TRexVpnSDK:get_root_ctx()
  return self._rootctx
end


function TRexVpnSDK:prepare(fetchargs)
...
function TRexVpnSDK:direct(fetchargs)
...
-- by passing it.
function TRexVpnSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TRexVpnSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "TRexVpnSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function TRexVpnSDK:Authentication(data)
...
function TRexVpnSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TRexVpnSDK.new(sdkopts)
  sdk.mode = "test"
